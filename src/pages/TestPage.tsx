...
export default function TestPage() {
  ...
  const {
    ...
    startTest, answerQuestion, goToQuestion, nextQuestion, prevQuestion,
    saveResult, getTimeTaken,
  } = useTest();
  ...
  useEffect(() => {
    fetchQuestions(10)
      ...
      .catch(() => {
        setErrorMsg('Failed to load questions. Is the backend running on port 5000?');
        setPageState('error');
      });
  }, [startTest]);
  ...
  const timerColor =
    timerPct > 0.5 ? 'text-brand-500' :
    timerPct > 0.2 ? 'text-yellow-400' : 'text-red-400';
  const timerBarColor =
    timerPct > 0.5 ? 'bg-brand-500' :
    timerPct > 0.2 ? 'bg-yellow-400' : 'bg-red-400';

  const difficultyClass =
    question.difficulty === 'easy' ? 'tag-green' :
    question.difficulty === 'hard' ? 'tag-red' : 'tag-yellow';
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col">
      ...
      <header className="border-b border-gray-800 px-4 sm:px-6 py-3 flex items-center gap-4">
        <button onClick={() => navigate('/')} className="btn-ghost text-sm flex-shrink-0">
          ← Exit
        </button>
        {/* Progress bar */}
        ...
        {/* Timer */}
        <span className={`font-mono font-bold text-lg flex-shrink-0 tabular-nums ${timerColor}`}>
          {mins}:{secs}
        </span>
      </header>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8 animate-fade-in">
        ...
        <div className="card p-6 sm:p-8 mb-6">
          <div className="flex flex-wrap gap-2 mb-5">
            <span className={`tag ${difficultyClass}`}>{question.difficulty}</span>
      ...
      {showConfirm && (
        <ConfirmModal
          ...
          onConfirm={() => { setShowConfirm(false); doSubmit(true); }}
      ...
      {errorMsg && pageState === 'ready' && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-red-500 text-white
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="card p-6 max-w-sm w-full animate-slide-up">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} className="btn-primary flex-1">Submit Anyway</button>
        </div>
      </div>
    </div>
  );
