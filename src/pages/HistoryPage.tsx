...
export default function HistoryPage() {
  ...
  useEffect(() => {
    fetchHistory(user!._id)
      .then(setData)
      .catch(() =>
        setError("Failed to load your history. Is the backend running?"),
      )
    ...
    <div className="min-h-screen bg-gray-950 pb-16">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-10 animate-fade-in">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div
            className="w-14 h-14 rounded-2xl bg-brand-500/20 border border-brand-500/30
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-400 mt-0.5">
                    ...
                    Latest:{" "}
                    <strong
                      className={accuracyColor(trend.latestAttemptAccuracy)}
                    >
  ...
  return (
    <div className={`card p-5 ${isLatest ? "border-brand-500/30" : ""}`}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span
          className={`text-2xl font-bold font-mono ${accuracyColor(attempt.accuracy)}`}
        >
          {attempt.accuracy}%
        </span>
      </div>
      ...
      {attempt.weakAreas.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-3">
          ...
          {attempt.weakAreas.map((area) => (
            <span key={area} className="tag tag-yellow">
              {area}
            </span>
          ))}
        </div>
