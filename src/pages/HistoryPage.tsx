...
const TABS: Tab[] = [
  ...
  { id: "verbal", label: "Verbal", icon: "📖", color: "#60a5fa" },
  { id: "logical", label: "Logical", icon: "🧠", color: "#a78bfa" },
  { id: "abstract", label: "Abstract", icon: "🔷", color: "#f97316" },
];

const MOD_ICONS: Record<ModuleCategory, string> = {
  numerical: "🔢",
  verbal: "📖",
  logical: "🧠",
  abstract: "🔷",
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    ...
    fetchHistory(
      user!._id,
      activeTab === "all" ? null : (activeTab as ModuleCategory),
    )
      .then(setData)
      ...
      <main className="max-w-2xl mx-auto px-4 pt-10 animate-fade-in">
        ...
        {totalAttempts === 0 ? (
          ...
          <>
            {trend && (
              <div
                className={`card p-5 mb-6 flex items-center gap-4 border-l-4 ${trend.improving ? "border-brand-500" : "border-yellow-500"}`}
              >
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-400 mt-0.5">
                    ...
                    {" → "}Latest:{" "}
                    <strong className={accColor(trend.latestAttemptAccuracy)}>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  ...
                  >
                    ...
                    <Line
                      ...
                      stroke={lineColor}
                      strokeWidth={2.5}
                      dot={{ fill: lineColor, r: 5, strokeWidth: 0 }}
            ...
            <div className="space-y-3 mb-8">
              ...
              {[...history].reverse().map((attempt) => {
                ...
                return (
                  <div
                    key={attempt._id}
                    className={`card p-5 ${isLatest ? "border-brand-500/30" : ""}`}
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <span className="text-base">{modIcon}</span>
                      ...
                      <span
                        className={`text-2xl font-bold font-mono ${accColor(attempt.accuracy)}`}
                      >
                        {attempt.accuracy}%
                      </span>
                    </div>
                    <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden mb-3">
                      <div
                        className={`h-full rounded-full transition-all ${accBar(attempt.accuracy)}`}
